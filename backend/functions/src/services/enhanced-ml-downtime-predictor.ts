...
import { WeatherData, SolarActivityData, GPMRainfallData } from './weather-data-collector';

interface EnhancedMLTrainingFeatures extends MLTrainingFeatures {
...
class EnhancedMLDowntimePredictor {
  ...
  private trainingData: EnhancedMLTrainingFeatures[] = [];
  ...
  constructor() {
    ...
    this.featureWeights = {
      ...
      solarFlux: 0.20, // Enhanced weight for real solar flux
      sunspotNumber: 0.15, // New feature
      solarWindSpeed: 0.10, // New feature
      
      // Enhanced environmental features
      avgRainfall: 0.20,
      temperature: 0.05,
      humidity: 0.08, // New feature
      pressure: 0.05, // New feature
      windSpeed: 0.03, // New feature
      visibility: 0.05, // New feature
      gpmRainfall: 0.12, // Enhanced weight for real rainfall
      weatherQuality: -0.10, // New feature
  ...
  createEnhancedMLFeatures(
    satelliteFeatures: MLTrainingFeatures,
    weatherData: WeatherData | null,
    solarData: SolarActivityData | null,
    rainfallData: GPMRainfallData | null
  ): EnhancedMLTrainingFeatures {
    return {
      // Original satellite features
      ...
      solarFlux: solarData?.solarFlux || satelliteFeatures.solarFlux,
      geomagneticStorm: satelliteFeatures.geomagneticStorm,
      sunspotNumber: solarData?.sunspotNumber || 0,
      solarWindSpeed: solarData?.solarWindSpeed || 400,
      
      // Enhanced environmental features
      avgRainfall: satelliteFeatures.avgRainfall,
      temperature: weatherData?.temperature || satelliteFeatures.temperature,
      humidity: weatherData?.humidity || 50,
      pressure: weatherData?.pressure || 1013.25,
  ...
  async trainModel(trainingData: EnhancedMLTrainingFeatures[]): Promise<void> {
  ...
  private async performEnhancedGradientDescent(): Promise<void> {
    const baseLearningRate = 0.01;
    const epochs = 150; // More epochs for better convergence
    
    for (let epoch = 0; epoch < epochs; epoch++) {
      let totalError = 0;
      let featureUpdates: Record<string, number> = {};
  ...
  private calculateEnhancedScore(features: EnhancedMLTrainingFeatures): number {
    let score = 0.35; // Base probability - more realistic baseline (35% downtime risk)
  ...
  predictDowntime(features: EnhancedMLTrainingFeatures): ModelPrediction {
    if (!this.isTrained) {
  ...
  private calculateEnhancedConfidence(features: EnhancedMLTrainingFeatures): number {
    ...
    if (features.solarFlux > 0) confidence += 0.1;
    if (features.gpmRainfall > 0) confidence += 0.1;
    if (features.humidity > 0) confidence += 0.05;
    
    return Math.min(confidence, 1.0);
  }

  private calculateEnhancedFactorContributions(features: EnhancedMLTrainingFeatures): {
    spaceWeather: number;
    environmental: number;
    temporal: number;
    historical: number;
  ...
  private generateEnhancedRecommendation(probability: number, factors: any): string {
    if (probability > 0.7) {
  ...
  private calculateWeatherQuality(weatherData: WeatherData | null): number {
    if (!weatherData) return 0.5;
    
    let quality = 0.5;
    
    // Good visibility = higher quality
    if (weatherData.visibility > 10) quality += 0.2;
    else if (weatherData.visibility > 5) quality += 0.1;
    
    // Low cloud cover = higher quality
    if (weatherData.cloudCover < 30) quality += 0.2;
    else if (weatherData.cloudCover < 60) quality += 0.1;
    
    // Low precipitation = higher quality
    if (weatherData.precipitation < 1) quality += 0.1;
    
    return Math.max(0, Math.min(1, quality));
  }

  private getFeatureValue(features: EnhancedMLTrainingFeatures, featureName: string): number {
...
export const enhancedMLDowntimePredictor = new EnhancedMLDowntimePredictor();
